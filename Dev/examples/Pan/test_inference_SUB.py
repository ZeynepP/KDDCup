...
from utils.utils_mol import get_mol_features
from gnn import GNN
...
class OnTheFlyPCQMDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        data.mol_attr = torch.from_numpy(np.array(get_mol_features(smiles))).to(torch.float)
        data.idx = idx
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default="/home/zpehlivan/Ina/Data/KDD/",
                        help='data directory')
    parser.add_argument('--checkpoint_dir', type=str, default='', help='sdirectory to save checkpoint')
    ...
    smiles_dataset = PCQM4MDataset(root=args.data_dir, only_smiles=True)
